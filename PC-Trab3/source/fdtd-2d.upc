...
// UPC
shared [NX / THREADS] int nx;
shared [NY / THREADS] int ny;
shared [] int hz;
//shared nx, ny, hz;
...
static void init_array(int tmax,  
   ...
   for (i = 0; i < tmax; i++) _fict_[i] = (DATA_TYPE)i;
   for (i = 0; i < nx; i++)
      for (j = 0; j < ny; j++) {
...
int main(int argc, char** argv) {
   ...
   /* Initialize array(s). */
   init_array(tmax,  POLYBENCH_ARRAY(ex), POLYBENCH_ARRAY(ey),
   ...
   /* Run kernel. */
   kernel_fdtd_2d(tmax,  POLYBENCH_ARRAY(ex), POLYBENCH_ARRAY(ey),
                  POLYBENCH_ARRAY(hz), POLYBENCH_ARRAY(_fict_));
