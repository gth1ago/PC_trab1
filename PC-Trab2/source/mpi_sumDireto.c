...
void master(int world_rank, int count_worker) {
   ...
   while (fscanf(fin, "%c %ld\n", &action, &num) == 2) {
     if (action == 'e'){
         ...
         // verifica recebeu ocioso
         MPI_Test(&request, &ready, MPI_STATUS_IGNORE);
         if (ready) {
            printf("[Mestre] MPI_Test #1: message received (%d).\n", world_id);
            task_type *task = remove_q(&q);
            printf("[Mestre] Send to Worker (%d)\n", world_id);
            MPI_Send(&task->num, 1, MPI_LONG, world_id, 0, MPI_COMM_WORLD);
         } 
         else{
            printf("[Mestre] MPI_Test #1: message not received yet.\n");
            // tenta envia um msg p esse perdido
         }
   ...
   while (!EMPTY_Q(&q)) {
      ...
      if (task->action == 'p') {
         ...
         MPI_Send(&task->num, 1, MPI_LONG, world_id, 0, MPI_COMM_WORLD);
         printf("\n[Mestre] Send to Worker (%d) - Num: %ld\n", world_id, task->num);
      }
   }
   // TIME = 0.000173 
   printf("\nAvisar que finalizou\n\n");
